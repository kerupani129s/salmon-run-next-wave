...
- **(要確認)** N-ZAP85 ダメージ 28 から 30 へ変更 (対戦と共通)
	- 変更前: DPS = 28 × 60F / 5F = 336
	- 変更後: DPS = 30 × 60F / 5F = 360
- **(要確認)** ダイナモローラー 横振り 大ダメージ 角度 変更 (対戦と共通)
- ヴァリアブルローラー
	- **(要確認)** 横振り 大ダメージ 角度 変更 (対戦と共通)
	- **(要確認)** 縦振り インクロック時間 80F から 75F へ変更 (対戦と共通)
- パブロ 塗り進み
	- **(要確認)** 移動速度 変更 (対戦と共通)
	- **(要確認)** インク消費雨量 変更 **(対戦のみ？)**
- **(要確認)** パラシェルター 射撃 インク消費量 変更 **(対戦のみ？)**
- **(要確認)** スパイガジェット 射撃中 ジャンプ高度 変更 (対戦と共通)
- ボトルガイザー
	- **(要確認)** 初弾 インク消費量 2.2% から 2.4% へ変更 (対戦と共通)
		- 変更前: 射撃継続回数 = [100% / 2.2%] = 45
		- 変更後: 射撃継続回数 = [100% / 2.4%] = 41
		- 変更前: 射撃継続時間 = (45 - 1) × 8F × 1s / 60F = 5.9s
		- 変更後: 射撃継続時間 = (41 - 1) × 8F × 1s / 60F = 5.3s
	- **(要確認)** 連射 塗り 変更 (対戦と共通)
- **(要確認)** クーゲルシュライバー 長射程モード ブレ 変更 (対戦と共通)
- **(要確認)** ジムワイパー 縦斬り インク 幅 変更 (対戦と共通)
