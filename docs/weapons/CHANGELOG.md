...
- **(要確認)** プロモデラーMG 塗り 変更 (対戦と共通)
- **(要確認)** ホットブラスター インクロック時間 60F から 57F へ変更 (対戦と共通)
- **(要確認)** クラッシュブラスター 射撃中 移動速度 変更 (対戦と共通)
- **(要確認)** S-BLAST92 連射間隔 60F から 55F へ変更 (対戦と共通)
	- 変更前: 爆風 DPS = 150 × 60F / 60F = 150
	- 変更後: 爆風 DPS = 150 × 60F / 55F = 163.6
	- 変更前: 直撃 DPS = 300 × 60F / 60F = 300
	- 変更後: 直撃 DPS = 300 × 60F / 55F = 327.3
- カーボンローラー
	- **(要確認)** 横振り ダメージ減衰 変更 (対戦と共通)
	- **(要確認)** 縦振り 当たり判定 変更 (対戦と共通)
- **(要確認)** ダイナモローラー 横振り インク 飛び方 変更 (対戦と共通)
- **(要確認)** ワイドローラー 横振り ダメージ減衰 変更 **(対戦のみ？)**
- **(要確認)** パブロ 振り 当たり判定 変更 (対戦と共通)
- **(要確認)** ホクサイ 振り 当たり判定 変更 (対戦と共通)
- ガエンFF インク消費量
	- **(要確認)** 射撃 1.4％ から 1.2% へ変更 (対戦と共通)
		- 変更前: 射撃継続回数 = [100% / 1.4%] = 71
		- 変更後: 射撃継続回数 = [100% / 1.2%] = 83
		- 変更前: 通常 射撃継続時間 = (71 - 1) × 8F × 1s / 60F = 9.3s
		- 変更後: 通常 射撃継続時間 = (83 - 1) × 8F × 1s / 60F = 10.9s
	- **(要確認)** スライド 12% から 10% へ変更 (対戦と共通)
- **(要確認)** 24式張替傘・甲 シェルター 復活時間 150F から 130F へ変更 (対戦と共通)
	- 150F × 1s / 60F = 2.5s
	- 130F × 1s / 60F = 2.2s
...
- サメライド
	...
	- **(要確認)** 爆風 塗り 半径 変更 (対戦と共通)
- **(要確認)** テイオウイカ 突撃 チャージ時間 30F から 35F へ変更 (対戦と共通)
...
- パラシェルター シェルター 復活時間 390F から 330F へ変更 (対戦と共通)
	- 390F × 1s / 60F = 6.5s
	- 330F × 1s / 60F = 5.5s
- スパイガジェット シェルター 復活時間 4.5s から 4.0s へ変更 (対戦と共通)
	- 270F × 1s / 60F = 4.5s
	- 240F × 1s / 60F = 4.0s
