...
- **(要確認)** ボールドマーカー 弾速および射程 変更 (対戦と共通)
- **(要確認)** わかばシューター 弾速および射程 変更 (対戦と共通)
- **(要確認)** プロモデラーMG 弾速および射程 変更 (対戦と共通)
- N-ZAP85
	- **(要確認)** 射撃中 移動速度 変更 (対戦と共通)
	- **(要確認)** ブレ 変更 (対戦と共通)
- **(要確認)** スペースシューター ジャンプ中 ブレ 変更 (対戦と共通)
- **(要確認)** ワイドローラー 縦振り 当たり判定 変更 (対戦と共通)
- **(要確認)** フィンセント 連射間隔 15F から 13F へ変更 (対戦と共通)
	- 変更前: 前後隙除く DPS = 150 × 60F / 15F = 600
	- 変更後: 前後隙除く DPS = 150 × 60F / 13F = 692.3
- スプラスピナー
	- **(要確認)** 1 周チャージ チャージ時間 20F から 18F へ変更 (対戦と共通)
		- 変更前: チャージ時間含む チャージ時間含む DPS = 11 × 35 × 60F / (20F + 1F + (11 - 1) × 4F) = 378.7
		- 変更後: チャージ時間含む チャージ時間含む DPS = 11 × 35 × 60F / (18F + 1F + (11 - 1) × 4F) = 391.5
	- **(要確認)** フルチャージ チャージ時間 30F から 27F へ変更 (対戦と共通)
		- 変更前: チャージ時間含む チャージ時間含む DPS = 22 × 35 × 60F / (30F + 1F + (22 - 1) × 4F) = 401.7
		- 変更後: チャージ時間含む チャージ時間含む DPS = 22 × 35 × 60F / (27F + 1F + (22 - 1) × 4F) = 412.5
- バレルスピナー
	- **(要確認)** 射撃継続時間 変更 (対戦と共通)
	...
	- **(要確認)** 1 周チャージ チャージ時間 50F から 48F へ変更 (対戦と共通)
		- 変更前: チャージ時間含む DPS = 19 × 35 × 60F / (50F + 1F + (19 - 1) × 4F) = 324.4
		- 変更後: チャージ時間含む DPS = 21 × 35 × 60F / (48F + 1F + (21 - 1) × 4F) = 341.9
	- **(要確認)** フルチャージ チャージ時間 75F から 72F へ変更 (対戦と共通)
		- 変更前: チャージ時間含む DPS = 37 × 35 × 60F / (75F + 1F + (37 - 1) × 4F) = 353.2
		- 変更後: チャージ時間含む DPS = 41 × 35 × 60F / (72F + 1F + (41 - 1) × 4F) = 369.5
- **(要確認)** スパッタリー 弾速および射程 変更 (対戦と共通)
- **(要確認)** スプラマニューバー 通常 当たり判定 変更 (対戦と共通)
- 24式張替傘・甲
	- **(要確認)** 弾 塗り 幅 変更 (対戦と共通)
	- **(要確認)** インク消費量 変更 **(対戦のみ？)**
- **(要確認)** R-PEN/5H 塗り 幅 変更 (対戦と共通)
- デンタルワイパーミント
	- **(要確認)** 踏み込み 前隙 8F から 10F へ変更 (対戦と共通)
	- **(要確認)** 通常 塗り 幅 変更 (対戦と共通)

### スペシャルウェポン

- **(要確認)** ウルトラチャクチ 爆風 塗り 半径 変更 (対戦と共通)
...
- クマサン印のマニューバー 射撃 ダメージ 30 から 35 へ変更
...
- バイト専用 キャンピングシェルター
	...
	- 1 体当たり 最大ダメージ 245 から 360 へ変更
		...
		- 変更後: 1 体当たり DPS = 360 × 60F / 51F = 423.5
