...
- スクリュースロッシャー 連射間隔 33F から 38F へ変更 (対戦と共通)
	- 変更前: 直撃 DPS = 170 × 60 / 33 = 309.1
	- 変更後: 直撃 DPS = 170 × 60 / 38 = 268.4
	- 変更前: 渦 DPS = 100 × 60 / 33 = 181.8
	- 変更後: 渦 DPS = 100 × 60 / 38 = 157.9
- ジムワイパー 横 インク 幅 変更 (対戦と共通)
...
- バイト専用 ホクサイ インク消費量 3.2% から 3% へ変更 (対戦と共通)
	- 変更前: 射撃継続回数 = [100% / 3.2%] = 31
	- 変更後: 射撃継続回数 = [100% / 3.0%] = 33
- バイト専用 ワイドローラー 塗り進み ダメージ 70 から 100 へ変更
- バイト専用 R-PEN/5H
	- フルチャージ ダメージ 180 から 240 へ変更
		- 変更前: チャージ時間除く DPS = 180 × 60 / 17 = 635.3
		- 変更後: チャージ時間除く DPS = 240 × 60 / 17 = 847.1
		- 変更前: チャージ時間含む DPS = 5 × 180 × 60 / (72 + (5 - 1) × 17 + 1 + 1 + 4) = 369.9
		- 変更後: チャージ時間含む DPS = 5 × 240 × 60 / (72 + (5 - 1) × 17 + 1 + 1 + 4) = 493.2
...
- バイト専用 バケットスロッシャー ダメージ 100 から 140 へ変更
	- 変更前: DPS = 100 × 60 / 29 = 206.9
	- 変更後: DPS = 140 × 60 / 29 = 289.7
- バイト専用 キャンピングシェルター
	- 弾 ダメージ 35 から 45 へ変更
	- 1 体当たり 最大ダメージ 245 から 360 へ変更
		- 変更前: DPS = 245 × 60 / 51 = 288.2
		- 変更後: DPS = 360 × 60 / 51 = 423.5
	- シェルター ダメージ 100 から 150 へ変更
- バイト専用 スパイガジェット
	- 1 体当たり 最大ダメージ 120 から 150 へ変更
		- 変更前: DPS = 120 × 60 / 26 = 276.9
		- 変更後: DPS = 150 × 60 / 26 = 346.2
	- シェルター ダメージ 40 から 50 へ変更
- バイト専用 トライストリンガー 1 周チャージ ダメージ 75 から 100 へ変更
	- 変更前: DPS = 3 × 75  × 60 / (30 + 1 + 1 + 13) = 300
	- 変更後: DPS = 3 × 100 × 60 / (30 + 1 + 1 + 13) = 400
	- 変更前: 合計ダメージ = 3 × 75  = 225
	- 変更後: 合計ダメージ = 3 × 100 = 300
- バイト専用 LACT-450
	- ノーチャージ ダメージ 32 から 34 へ変更
		- 変更前: DPS = 3 × 32 × 60 / (6 + 1 + 1 + 6) = 411.4
		- 変更後: DPS = 3 × 34 × 60 / (6 + 1 + 1 + 6) = 437.1
		- 変更前: 合計ダメージ = 3 × 32 = 96
		- 変更後: 合計ダメージ = 3 × 34 = 102
	- 1 周チャージ ダメージ 50 から 70 へ変更
		- 変更前: DPS = 3 × 50 × 60 / (12 + 1 + 1 + 13) = 333.3
		- 変更後: DPS = 3 × 70 × 60 / (12 + 1 + 1 + 13) = 466.7
		- 変更前: 合計ダメージ = 3 × 50 = 150
		- 変更後: 合計ダメージ = 3 × 70 = 210
...
- メガホンレーザー5.1ch
	- 追尾 変更
	- ダメージ 6 から 7.5 へ変更
		- 変更前: 1 対 DPS = 2 × 6 × 60 / 5 = 144
		- 変更後: 1 対 DPS = 2 × 7.5 × 60 / 5 = 180
		- 変更前: 1 対 合計ダメージ = 2 × 6 × 180 / 5 = 432
		- 変更後: 1 対 合計ダメージ = 2 × 7.5 × 180 / 5 = 540
- トリプルトルネード
	- 起動 変更
	- ダメージ 40 から 50 へ変更
		- 変更前: 1 つ DPS = 40 × 60 / 5 = 480
		- 変更後: 1 つ DPS = 50 × 60 / 5 = 600
