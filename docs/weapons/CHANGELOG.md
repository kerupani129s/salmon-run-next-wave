...
- スペースシューター 弾速 変更 (対戦と共通)
- ヴァリアブルローラー 縦振り ダメージ減衰 変更 (対戦と共通)
- ホクサイ 振り インク消費量 3.0% から 2.7% へ変更 (対戦と共通)
	- 変更前: 射撃継続回数 = [100% / 3.0%] = 33
	- 変更後: 射撃継続回数 = [100% / 2.7%] = 37
	- 変更前: 射撃継続時間 = (33 - 1) × 11F × 1s / 60F = 5.9s
	- 変更後: 射撃継続時間 = (37 - 1) × 11F × 1s / 60F = 6.6s
- 14式竹筒銃・甲
	- チャージ中 移動速度 変更 (対戦と共通)
	- 射撃中 移動速度 変更 (対戦と共通)
- ガエンFF スライド後
	- **(要確認)** ブキ使用禁止時間 6F から 4F へ変更 (対戦と共通)
	- **(要確認)** 硬直時間 42F から 40F へ変更 (対戦と共通)
- キャンピングシェルター パージ インク消費量 30% から 23% へ変更 (対戦と共通)

## [11.0.1] - 2026-02-05

### メインウェポン

- **(要確認)** ラピッドブラスター 塗り 変更 (対戦と共通)

## [11.0.0] - 2026-01-29

### メインウェポン

- **(要確認)** 全て 当たり判定 変更 (対戦と共通)
- L3リールガン
	- 弾速 変更 (対戦と共通)
	- 塗り 変更 (対戦と共通)
- ダイナモローラー
	- 塗り 変更 (対戦と共通)
	- **(要確認)** ダメージ減衰 変更 (対戦と共通)
- ホットブラスター 後隙 23F から 21F へ変更 (対戦と共通)
- スプラローラー インクロック時間 45F から 43F へ変更 (対戦と共通)
- バレルスピナー 塗り 変更 (対戦と共通)
- クアッドホッパーブラック 弾速 変更 (対戦と共通)
- S-BLAST92 短射程モード 遠爆風 ダメージ 半径 変更 (対戦と共通)
...
- クラッシュブラスター 塗り 変更 (対戦と共通)
- ガエンFF 弾速 変更 (対戦と共通)
- **(要確認)** 24式張替傘・甲 シェルター ダメージ補正 変更 (対戦と共通)
...
- 24式張替傘・甲 弾 塗り 変更 (対戦と共通)
- R-PEN/5H 塗り 変更 (対戦と共通)
- デンタルワイパーミント
